...
% Input image
if ~exist('im', 'var')
    im = imread('E:\SolidDetectorImages\musica\Shutter\1.3.76.6.1.1.5.2.D.145580345645553.Dat.png');
    im = double(im);
    im = im / max(im(:));
end
...
% Parameteters:
nlevels = [0, 3, 2, 2, 2];   % Decomposition level
...
if strcmp( option, 'auto' )
    nSignif = round(nPixels * 2. / 100) ;  % 2.5% of coefficients
...
subplot(1,2,1), imagesc ( im_wrec, [0, 1] ) ; 
