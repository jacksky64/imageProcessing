...
const float GaussianPyramid::kA = 0.4f;
...
GaussianPyramid::GaussianPyramid(const Mat& image, int num_levels,
  ...
  image.convertTo(pyramid_.back(), CV_32F);
  ...
  for (int l = 0; l < num_levels; l++) {
    ...
    if (next.channels() == 1) {
      PopulateTopLevel<float>(row_offset, col_offset);
...
Mat GaussianPyramid::Expand(int level, int times) const {
  ...
  for (int i = 0; i < times; i++) {
    ...
    if (base.channels() == 1) {
      Expand<float>(base, row_offset, col_offset, expanded);
    } else {
...
inline float GaussianPyramid::WeightingFunction(int i, float a) {
  switch (i) {
    case 0: return a;
