...
int main(int argc, char** argv) {
	...
	cv::Mat input = cv::imread(argv[1], cv::IMREAD_GRAYSCALE | cv::IMREAD_ANYDEPTH);
	if (input.data == NULL) {
	...
	input.convertTo(input, CV_32F, 1 / maxVal);
	...
	cv::Mat output;
	if (input.channels() == 1) {
		output = LocalLaplacianFilter<float>(input, kAlpha, kBeta, kSigmaR);
	}
