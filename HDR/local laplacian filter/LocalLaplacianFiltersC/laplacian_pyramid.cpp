...
LaplacianPyramid::LaplacianPyramid(int rows,
  ...
  for (int i = 0; i < num_levels + 1; i++) {
    pyramid_.emplace_back((int)ceil(rows / (float)(1 << i)),
		(int)ceil(cols / (float)(1 << i)), CV_32FC(channels));
  }
}
...
Mat LaplacianPyramid::Reconstruct() const {
  ...
  for (int i = pyramid_.size() - 2; i >= 0; i--) {
    ...
    if (base.channels() == 1) {
      GaussianPyramid::Expand<float>(base, row_offset, col_offset, expanded);
...
int LaplacianPyramid::GetLevelCount(int rows, int cols, int desired_base_size) {
  ...
  float log2_dim = std::log2(min_dim);
  float log2_des = std::log2(desired_base_size);
