function varargout=elastixMasked(movingImage,fixedImage,fixedMask,outputDir,paramFile,varargin)
% elastix image registration and warping wrapper
%
% function varargout=elastix(movingImage,fixedImage,fixedMask,outputDir,paramFile)
%          varargout=elastix(movingImage,fixedImage,fixedMask,outputDir,paramFile,'PARAM1',val1,...)
% Purpose
...
%
% fixedMask -   A 2D or 3D matrix corresponding to a 2D image or a 3D volume. 
%               Must have the same number of dimensions as fixedImage.
%               This is the mask where samples are taken               
%
...
if ~isempty(fixedMask) && isnumeric(fixedMask)
    fixedMaskFname=[dirName,'_fixedmask'];
    mhd_write(fixedMask,fixedMaskFname);
    if ~strcmp(outputDir,'.') %Don't copy if we're already in the directory
        if ~movefile([fixedMaskFname,'.*'],outputDir); error('Can''t move files'), end
    end
end
